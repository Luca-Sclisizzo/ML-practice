# -*- coding: utf-8 -*-
"""
Created on Mon Aug 26 12:22:33 2024
...
# %% Section 8: Plotting the surface plot with 100 trees
#  Defining the axes
...
mse_plot = mean_squared_error(y_train, model_100.predict(filtered_df))
print('R^2 del plot', R2_plot)
print('MSE del plot', mse_plot)
...
'''
# %% Section 9: Accounting for the mistake made above
# splitting into two datsets to train and test the model
x_train_feat, x_test_feat, y_train_feat, y_test_feat = train_test_split(
        filtered_df, y_train,
        test_size=0.2
        )

model_fitted = model_100.fit(x_train_feat, y_train_feat)  # Fit the model
prediction_feat = model_fitted.predict(x_test_feat)  # Make the prediction
r2_feat = r2_score(y_test_feat, prediction_feat)  # Test the fit of the model
mse_feat = mean_squared_error(y_test_feat, prediction_feat)
if r2_feat < r2:  # Compare the fit of this model with the other fits
    print("""
          L'R^2 del modello con 8 features è maggiore di quello con 2 feature
          """)
else:
    print("C'è qualcosa che non va")

print("L'R^2 del modello con 8 features è:", r2)
print("L'R^2 del modello con 2 features errato è:", R2_plot)
print("L'R^2 del modello con 2 features corretto è:", r2_feat)

'''
I realized that the inflated R² in my model was due to a conceptual mistake:
I was evaluating the model's performance using the same dataset that I trained
it on. This can lead to overfitting, where the model learns patterns specific
to the training data, resulting in an artificially high R².

To correct this, I split the feature_df dataset into separate training and
testing sets. By training the model on one portion of the data and evaluating
it on the other, I ensured that the R² value reflects the model's true ability
to generalize to new, unseen data, providing a more accurate and realistic
